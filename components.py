...
def header(request, title, book=None, item=None, tools=None, search=True):
            ...
            if item is None:
                menu.extend(
                    [
                        ...
                        A(Tx("Download book TGZ file"), href=f"/download/{book}"),
    ...
    if item:
        nav_style = f"outline-color: {item.status.color};"
    elif book:
        nav_style = f"outline-color: {book.status.color};"
    else:
        nav_style = "outline-color: black;"
